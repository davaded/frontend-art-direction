...
- **Typography**: Set readable type scale, weight, line height, numeric treatment, labels, and content tone from foundation references or project tokens. Use typographic contrast and rhythm to create character. Avoid oversized type as a substitute for design. In product tools, prefer dense, role-based type over hero-scale headings unless the reference evidence explicitly supports a display moment and the screen has enough real content or visual media to carry it.
...
- Inspect type scale specifically: no hero-sized heading, giant number, or marketing headline should dominate a compact product surface unless the locked typography reference, product context, and supporting content/media justify it.
...
- a display-support check when large type is used: name the content, image, animation, model, data object, or editorial material that makes the scale feel earned
