...
- Visual composition and authority: `scripts/visual-direction.mjs`, `scripts/authority.mjs`, `data/visual-directions.json`, `data/visual-treatments.json`, `data/constraint-policy.json`, and `references/visual-composition.md` separate content structure from visual expression, then protect hard invariants while keeping AI-default checks overrideable by project-owned, user-owned, reference-led, concept-led, or evidence-backed model direction.
...
| Visual direction and authority | `direction`, `brief`, `reference-build`, `audit` | named composition grammar, authority source, hard invariants, overrideable defaults, type/spacing/surface rules, rendered proof |
...
- Every visual-direction record needs a first-viewport contract, type/spacing/surface rules, anti-AI checks, and rendered checks.
