...
export const REFERENCE_COVERAGE = [
  ...
  {
    id: "geometry-language",
    capability: "soft structural geometry, corner hierarchy, non-border separation, line policy, and justified sharp exceptions",
    implementation: "references/geometry-language.md + data/visual-directions.json + data/quality-gates.json",
...
function buildControlDials(brief, referenceBuild) {
  ...
  return {
    ...
    edgeCharacter: brief.visualDirection?.geometryRules?.edgeCharacter ?? "not resolved",
    cornerHierarchy: brief.visualDirection?.geometryRules?.cornerHierarchy ?? "not resolved",
    separationStrategy: brief.visualDirection?.geometryRules?.separation ?? "not resolved",
    linePolicy: brief.visualDirection?.geometryRules?.linePolicy ?? "not resolved",
