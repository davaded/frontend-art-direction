...
- a Requirement Frame and Local Evidence Scan for substantial work: surface type, workflow, density, local components/tokens/assets, weak primitives, and gaps to fill
...
- an evidence pack when the work is substantial: requirement frame, local evidence used, references used, components/resources used, largest text role/size, viewport or screenshot check, motion trigger, and reduced-motion or fallback note
...
When feedback says the result has no beauty, no coordination, poor interaction, poor visuals, no components, or no reference-site influence, treat it as an execution failure, not a subjective disagreement. Re-run the Reference Website Pass and Component Adoption Plan before continuing, replace weak freehand primitives with mature components or stronger local primitives, and verify where references/components/motion are visible in the rendered result.
